...
if (window._applyWiseLoaded) {
  ...
  function handleSave(saveBtn, statusBar, fields) {
    ...
    chrome.runtime.sendMessage({ action: 'getGoogleToken' }, function(response) {
      ...
      console.log("[ApplyWise] Got Google user:", userEmail);
      showStatus(statusBar, '🌐 Saving via ApplyWise API...', 'info');
      setBtnState(saveBtn, true, '⏳ Saving...');

      // ── Build the job document — matching Dashboard.jsx fields
      // Note: userId/email are derived server-side from the JWT token.
      var jobDoc = {
      ...
      if (!window.ApplyWiseJobsApi || !window.ApplyWiseJobsApi.createJob) {
        console.error("[ApplyWise] Jobs API helper missing. Check manifest content_scripts order.");
        showStatus(statusBar, '❌ Internal error: jobs API helper not loaded', 'error');
        setBtnState(saveBtn, false);
        return;
      }

      window.ApplyWiseJobsApi.createJob(jobDoc)
        .then(function(resp) {
          console.log("[ApplyWise] Saved via API:", resp && resp.data ? resp.data.id : resp);
        ...
        .catch(function(err) {
          ...
          // Requirement: if token missing, log error and skip request.
          // We also surface a helpful status message.
          showStatus(statusBar, '❌ Failed: ' + (err && err.message ? err.message : 'unknown error'), 'error');
          setBtnState(saveBtn, false);
        });
    });
  }
