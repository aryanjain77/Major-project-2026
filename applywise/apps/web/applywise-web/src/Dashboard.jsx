// src/Dashboard.jsx
import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { auth } from './firebase';
import { getJobs, createJob, deleteJob } from './api/jobsApi';
import {
  Chart as ChartJS,
  CategoryScale,
  LinearScale,
  BarElement,
  Title,
  Tooltip,
  Legend,
} from 'chart.js';
import { Bar } from 'react-chartjs-2';
import ChartDataLabels from 'chartjs-plugin-datalabels';

ChartJS.register(CategoryScale, LinearScale, BarElement, Title, Tooltip, Legend, ChartDataLabels);

// ─── Helper: safely format any date value from Firebase ───────────────────────
const formatDate = (val) => {
  if (!val) return 'N/A';
  if (typeof val === 'string') return val;
  if (val?.toDate) return val.toDate().toLocaleDateString();
  if (val?.seconds) return new Date(val.seconds * 1000).toLocaleDateString();
  return 'N/A';
};

function Dashboard() {
  const navigate = useNavigate();
  const [jobs, setJobs] = useState([]);
  const [loading, setLoading] = useState(true);
  const [refreshKey, setRefreshKey] = useState(0); // used to force re-fetch

  // ─── Fetch jobs — waits for Firebase auth to be ready ──────────────────────
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      if (!user?.email) {
        setLoading(false);
        return;
      }

      setLoading(true);
      try {
        const resp = await getJobs();
        const userJobs = resp?.data || [];

        // Sort by timestamp descending (newest first)
        userJobs.sort((a, b) => {
          const aTime = a.timestamp ? new Date(a.timestamp).getTime() : 0;
          const bTime = b.timestamp ? new Date(b.timestamp).getTime() : 0;
          return bTime - aTime;
        });

        setJobs(userJobs);
      } catch (error) {
        console.error("Error fetching jobs:", error);
        alert("Could not load your jobs. Check console.");
      } finally {
        setLoading(false);
      }
    });

    return () => unsubscribe(); // cleanup on unmount
  }, [refreshKey]); // re-runs when refreshKey changes (after add/delete)

  // ─── Funnel chart ───────────────────────────────────────────────────────────
  const statusOrder = ['Applied', 'OA', 'Technical', 'HR', 'Offer', 'Rejected'];

  const statusCounts = jobs.reduce((acc, job) => {
    const status = job.status?.trim() || 'Applied';
    acc[status] = (acc[status] || 0) + 1;
    return acc;
  }, {});

  const chartData = statusOrder.map(status => statusCounts[status] || 0);
  const total = chartData.reduce((sum, val) => sum + val, 0) || 1;

  const funnelData = {
    labels: statusOrder,
    datasets: [{
      label: 'Applications',
      data: chartData,
      backgroundColor: [
        '#2563eb', // Applied
        '#3b82f6', // OA
        '#60a5fa', // Technical
        '#93c5fd', // HR
        '#10b981', // Offer
        '#ef4444', // Rejected
      ],
      borderWidth: 1,
    }],
  };

  const funnelOptions = {
    indexAxis: 'y',
    responsive: true,
    maintainAspectRatio: false,
    plugins: {
      legend: { display: false },
      title: {
        display: true,
        text: 'Your Application Funnel',
        font: { size: 18 },
        padding: { top: 10, bottom: 20 }
      },
      tooltip: {
        callbacks: {
          label: function (context) {
            const value = context.raw;
            const percentage = total > 0 ? ((value / total) * 100).toFixed(0) : 0;
            return `${context.label}: ${value} applications (${percentage}%)`;
          }
        }
      },
      datalabels: {
        anchor: 'end',
        align: 'end',
        color: '#333',
        font: { weight: 'bold', size: 12 },
        formatter: (value) => value > 0 ? value : ''
      }
    },
    scales: {
      x: {
        beginAtZero: true,
        min: 0,
        suggestedMax: Math.max(...chartData, 1) + 2,
        title: { display: true, text: 'Number of Applications' },
        ticks: { stepSize: 1 }
      },
      y: {
        title: { display: true, text: 'Stage' },
        ticks: { autoSkip: false, maxRotation: 0, minRotation: 0 }
      }
    },
    animation: { duration: 800 }
  };

  // ─── Delete job ─────────────────────────────────────────────────────────────
  const handleDelete = async (job) => {
    if (!window.confirm(`Delete ${job.company} - ${job.role}?`)) return;
    try {
      await deleteJob(job.id);
      setJobs(prev => prev.filter(j => j.id !== job.id));
    } catch (err) {
      alert('Delete failed: ' + err.message);
    }
  };

  // ─── Add job ────────────────────────────────────────────────────────────────
  const handleAddJob = async (e) => {
    e.preventDefault();
    const formData = new FormData(e.target);

    const newJob = {
      userId: auth.currentUser.uid,
      email: auth.currentUser.email,
      dateApplied: formData.get('dateApplied') || new Date().toISOString().split('T')[0],
      company: formData.get('company')?.trim(),
      role: formData.get('role')?.trim(),
      status: formData.get('status'),
      stipend: formData.get('stipend')?.trim() || '',
      description: formData.get('description')?.trim() || '',
      link: formData.get('link')?.trim() || '',
      notes: formData.get('notes')?.trim() || '',
      timestamp: new Date().toISOString()
    };

    if (!newJob.company || !newJob.role || !newJob.status) {
      alert('Please fill Company, Role, and Status');
      return;
    }

    try {
      const resp = await createJob(newJob);
      // Add to local state immediately (no need to re-fetch)
      const created = resp?.data ? resp.data : { ...newJob };
      setJobs(prev => [{ id: created.id, ...created }, ...prev]);
      e.target.reset();
      alert('Job added successfully!');
    } catch (error) {
      console.error("Add job error:", error);
      alert('Failed to add job. Check console.');
    }
  };

  // ─── Render ─────────────────────────────────────────────────────────────────
  return (
    <div className="dashboard">
      <h2>Your Dashboard</h2>
      <p>Track your applications and see where you're at.</p>

      {/* Refresh button */}
      <button
        className="btn btn-secondary"
        onClick={() => setRefreshKey(k => k + 1)}
        style={{ marginBottom: '16px' }}
      >
        🔄 Refresh
      </button>

      {/* Job List Table */}
      <div className="card">
        <h3>Recent Applications ({jobs.length})</h3>
        {loading ? (
          <p>Loading your jobs...</p>
        ) : jobs.length === 0 ? (
          <p>No applications yet. Add one below!</p>
        ) : (
          <table className="job-table">
            <thead>
              <tr>
                <th>Date</th>
                <th>Company</th>
                <th>Role</th>
                <th>Status</th>
                <th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {jobs.map(job => (
                <tr key={job.id}>
                  <td>{formatDate(job.dateApplied)}</td>
                  <td>{job.company || 'N/A'}</td>
                  <td>{job.role || 'N/A'}</td>
                  <td className={`status ${job.status?.toLowerCase() || 'applied'}`}>
                    {job.status || 'Applied'}
                  </td>
                  <td>
                    <button
                      onClick={() => handleDelete(job)}
                      style={{
                        background: 'none', border: '1px solid #fca5a5',
                        color: '#dc2626', borderRadius: '5px',
                        padding: '3px 10px', cursor: 'pointer', fontSize: '12px'
                      }}
                    >
                      Delete
                    </button>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        )}
      </div>

      {/* Funnel Chart */}
      <div className="card" style={{ height: '320px' }}>
        <Bar data={funnelData} options={funnelOptions} />
      </div>

      {/* Add Job Form */}
      <div className="card">
        <h3>Add New Job</h3>
        <form onSubmit={handleAddJob} className="add-job-form">
          <div className="form-group">
            <label>Date Applied</label>
            <input
              type="date"
              name="dateApplied"
              defaultValue={new Date().toISOString().split('T')[0]}
            />
          </div>

          <div className="form-group">
            <label>Company</label>
            <input type="text" name="company" placeholder="e.g. Google" required />
          </div>

          <div className="form-group">
            <label>Role</label>
            <input type="text" name="role" placeholder="e.g. Software Engineer" required />
          </div>

          <div className="form-group">
            <label>Status</label>
            <select name="status" required>
              <option value="">Select status</option>
              <option value="Applied">Applied</option>
              <option value="OA">OA</option>
              <option value="Technical">Technical</option>
              <option value="HR">HR</option>
              <option value="Offer">Offer</option>
              <option value="Rejected">Rejected</option>
            </select>
          </div>

          <div className="form-group">
            <label>Stipend/Package</label>
            <input type="text" name="stipend" placeholder="e.g. 20-30 LPA" />
          </div>

          <div className="form-group">
            <label>Link</label>
            <input type="url" name="link" placeholder="https://..." />
          </div>

          <div className="form-group">
            <label>Notes</label>
            <textarea name="notes" placeholder="Any extra notes..." rows="3" />
          </div>

          <button type="submit" className="btn btn-primary">
            Add Job
          </button>
        </form>
      </div>

      <button
        className="btn btn-primary"
        onClick={() => navigate('/')}
      >
        Back to Home
      </button>
    </div>
  );
}

export default Dashboard;